 ...
 */
async function runUserCommands(message: Message, statement: string, server: LocalServer, args: Array<string>, prefixString: string) {
  ...
  case 'renamekeys':
    ...
    commandHandlerCommon.renameKey(<TextChannel>message.channel, server, getSheetName(message.member!.id), args[1],
      args[2]).then();
    break;
  ...
  case 'renameplaylist':
    ...
    commandHandlerCommon.renamePlaylist(<TextChannel>message.channel, server, getSheetName(message.member!.id), args[1],
      args[2]).then();
    break;
  ...
  case 'playlists':
    commandHandlerCommon.keys(message, server, getSheetName(message.member!.id), null, args[1],
      message.member!.nickname ?? undefined).then();
    break;
  case 'splash':
    commandHandlerCommon.setSplashscreen(server, <TextChannel>message.channel, getSheetName(message.member!.id), args[1]).then();
    break;
  ...
  case 'queuefind':
    ...
    commandHandlerCommon.queueFind(message, server, args.slice(1).join(' ')).catch((er: Error) => processStats.debug(er));
    break;
  case 'audit':
  case 'freq':
  case 'frequency':
  ...
  case 'playlist-delete':
    commandHandlerCommon.removeDBPlaylist(server, getSheetName(message.member!.id), args[1],
      (await getXdb2(server, getSheetName(message.member!.id), false)), <TextChannel>message.channel).then();
    break;
  ...
  case 'move-keys':
    ...
    commandHandlerCommon.moveKeysBetweenPlaylists(server, <TextChannel>message.channel, getSheetName(message.member!.id),
      (await getXdb2(server, getSheetName(message.member!.id), false)), args.splice(1));
    break;
  case 'soundcloud':
 ...
 * @returns {Promise<void>}
 */
async function runDevCommands(message: Message, statement: string, server: LocalServer, args: Array<string>, prefixString: string) {
  const mgid = message.guild!.id;
  switch (statement) {
  case 'gztest':
    // =gztest
  ...
  case 'gplaylist-delete':
    commandHandlerCommon.removeDBPlaylist(server, 'entries', args[1],
      (await getXdb2(server, 'entries', false)), <TextChannel>message.channel).then();
    break;
  ...
  case 'gmove-keys':
    if (!args[1]) return message.channel.send(`*no args provided (i.e. ${statement} [key] [playlist])*`);
    commandHandlerCommon.moveKeysBetweenPlaylists(server, <TextChannel>message.channel, 'entries', (await getXdb2(server, 'entries', false)), args.splice(1));
    break;
  case 'gnow':
  case 'g?':
  ...
  case 'gzsms':
    if (args[1]) {
      ...
      processStats.servers.forEach((x: LocalServer) => {x.startUpMessage = ''})
  ...
  case 'gzm':
    if (!args[1]) {
        ...
        bot.voice.adapters.forEach((x: any, gId: string) => {
          try {
            ...
            if (currentTextChannel) {
              ...
              bot.channels.cache.get(getVoiceConnection(gId)?.joinConfig.channelId).guild.systemChannel.send(updateMsg);
            }
          }
          catch (e) {}
        });
      ...
      bot.voice.adapters.forEach((x:any, g:any) => {
        try {
          // guild member array
          const gmArray: Array<[Snowflake, GuildMember]> = Array.from(bot.channels.cache.get(getVoiceConnection(g)!.joinConfig.channelId).members);
