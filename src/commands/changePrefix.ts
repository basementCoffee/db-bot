      ...
      gsrun('A', 'B', PREFIX_SN).then(async (xdb2) => {
        ...
        async function changeNamePrefix() {
          if (!botObject.nickname) {
            ...
            await botObject.setNickname(
              '[' + newPrefix + '] ' + botObject.nickname.substring(botObject.nickname.indexOf(']') + 2)
            );
          } else {
        ...
        if (
          !botObject.nickname ||
          (botObject.nickname.substring(0, 1) !== '[' && botObject.nickname.substr(2, 1) !== ']')
        ) {
          message.channel
            .send(
              '----------------------\nShould I update my nickname to display this change? (yes or no)\nFrom **' +
                getBotDisplayName(message.guild!) +
                '**  -->  **[' +
                newPrefix +
                '] ' +
                name +
            ...
            .then(() => {
              ...
              message.channel
                ...
                .then(async (messages) => {
                  ...
                  if (
                    messages.first() &&
                    (messages.first()!.content.toLowerCase() === 'yes' ||
                      messages.first()!.content.toLowerCase() === 'y')
                  ) {
