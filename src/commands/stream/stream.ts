...
import {
  Message, MessageReaction, TextChannel, User, VoiceBasedChannel,
} from "discord.js";
...
import { stopPlayingUtil, voteSystem, pauseCommandUtil, endAudioDuringSession, playCommandUtil, pauseComputation,
  playComputation,
} from './utils';
 ...
 */
async function playLinkToVC(message: Message, queueItem: any, vc: VoiceBasedChannel | null | undefined, server: LocalServer, retries = 0, seekSec = 0): Promise<any> {
  if (!vc) {
  ...
  if (server.numSinceLastEmbed > 4 && server.currentEmbed &&
    (!server.loop || whatspMap.get(vc.id) !== whatToPlay)) {
  ...
  try {
    ...
    server.audio.player!.on('error', async (error) => {
      ...
      logError(
        ...
        timestamp: ${formatDuration(resource.playbackDuration)}\nprevSong: ${server.queueHistory[server.queueHistory.length - 1]?.url}`).build()],
        },
      );
 ...
 */
async function skipLink(message: Message, voiceChannel: VoiceBasedChannel, playMessageToChannel: boolean, server: LocalServer, noHistory?: boolean) {
 ...
 * @returns {*}
 */
function runRewindCommand(message: Message, mgid: string, voiceChannel: VoiceBasedChannel, numberOfTimes: string, ignoreSingleRewind: boolean, force: boolean, mem: any, server: LocalServer) {
  if (!voiceChannel) {
 ...
 * @returns {*}
 */
async function runSkipCommand(message: Message, voiceChannel: VoiceBasedChannel | null, server: LocalServer, skipTimes: any, sendSkipMsg: any, forceSkip: boolean, mem: any) {
 ...
 */
async function sendLinkAsEmbed(message: Message, queueItem: QueueItem, voiceChannel: VoiceBasedChannel | undefined | null, server: LocalServer, forceEmbed: boolean) {
  ...
  if (botInVC(message) && (server.queue.length < 1 || server.queue[0]?.url === url)) {
    ...
    await sendEmbedUpdate(<TextChannel>message.channel, server, forceEmbed, embed).then((sentMsg) => {
      if (server.audio.player) {
        generatePlaybackReactions(sentMsg, server, voiceChannel!, timeMS, message.guild!.id);
      }
 ...
 */
async function sendEmbedUpdate(channel: TextChannel, server: LocalServer, forceEmbed: boolean, embed: EmbedBuilderLocal) {
  server.numSinceLastEmbed = 0;
  if (server.currentEmbed) {
 ...
 * @param mgid The message guild id
 */
async function generatePlaybackReactions(sentMsg: Message, server: LocalServer, voiceChannel: VoiceBasedChannel, timeMS: number, mgid: string) {
  if (!sentMsg) return;
  ...
  collector.on('collect', async (reaction: MessageReaction, user: User) => {
    ...
    case reactions.REWIND:
      ...
      runRewindCommand(sentMsg, mgid, voiceChannel, '1', true,
        false, sentMsg.member!.voice?.channel?.members.get(user.id), server);
      if (server.followUpMessage) {
        server.followUpMessage.delete();
        server.followUpMessage = undefined;
      }
    ...
    case reactions.BOOK_O:
      ...
      runKeysCommand(sentMsg, server,
        getSheetName(user.id.toString()), user, undefined, tempUserBook.nickname).then();
