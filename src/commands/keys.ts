import {
  ...
  MessagePayload, MessageReaction,
  TextChannel,
  User
} from "discord.js";
...
const getTips = (prefixS: string) => ['click on the arrow keys!', 'the gear icon is page-specific',
 ...
 */
async function createKeyEmbedPages(title: string, keyEmbedColor: ColorResolvable, prefixString: string, xdb: any, server: LocalServer, sheetName: string, iconUrl?: string): Promise<Array<EmbedBuilderLocal>> {
  let playlistString = '';
  let counter = 1;
  const embedPages = [];
  ...
  embedPages.push(
      new EmbedBuilderLocal()
          ...
          .setFooter({text: `${getTips(prefixString)[Math.floor(Math.random() * getTips(prefixString).length)]}`, iconURL: iconUrl})
 ...
 */
async function runKeysCommand(message: Message, server: LocalServer, sheetName: string, user: any | null, specificPage?: string, overrideName?: string | null) {
  if (!user) user = message.member!.user;
  const keysMsg = (botInVC(message) ? {edit: (editOptions: string | MessagePayload | MessageCreateOptions) => message.channel.send(editOptions) } :
  ...
  if (keyArray.length < 1) {
    ...
    const keyEmbedColors: Array<ColorResolvable> = ['#cdfc41', '#4192fc', '#fc4182', '#41fc9f', '#47fc41', '#41ecfc', '#90d5cf'];
    ...
    const generateKeysEmbed = async (requiresUpdate: boolean) => {
      ...
      if (specificPage) {
        ...
        if (index === -1) {
          const ss = getAssumptionMultipleMethods(specificPage, <string[]>Array.from(xdb.playlists, ([name2]) => name2));
      ...
      keysButtonCollector.on('collect', async (reaction, reactionCollector) => {
        if (reaction.emoji.name === reactions.QUESTION) {
          ...
          if (reaction.emoji.name === reactions.ARROW_R) {
            ...
            await reaction.users.remove(user.id);
            const wasChanged = await addRemoveWizard(<TextChannel>message.channel, user, server,
              await getXdb2(server, sheetName, false), pageIndex, sheetName);
 ...
 */
async function addRemoveWizard(channel: TextChannel, user: User, server: LocalServer, xdb: any, pageIndex: number, sheetName: string) {
  ...
  if (pageIndex) {
    ...
    renameFunction = async (oldName: string, newName: string) => renameKey(channel, server, sheetName, oldName, newName);
    ...
    renameFunction = async (oldName: string, newName: string) => renamePlaylist(channel, server, sheetName, oldName, newName);
 ...
 */
async function addKeyWizard(channel: TextChannel, user: User, server: LocalServer, xdb: any, playlistName: string, sheetName: string) {
 ...
 */
async function renameWizard(channel: TextChannel, user: User, server: LocalServer, xdb: any, type: string, renameFunction: any) {
 ...
 */
async function removePlaylistWizard(channel: TextChannel, user: User, server: LocalServer, xdb: any, sheetName: string): Promise<boolean> {
 ...
 */
async function getMessageResponse(server: LocalServer, channel: TextChannel, user: User, sentMsg: Message): Promise<string | undefined> {
