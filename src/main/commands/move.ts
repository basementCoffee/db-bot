  ...
  if (!(posA && posB)) {
    channel.send(
      "*two numbers expected: the position of the item to move and it's new position*\n`ex: move 1 5`\n" +
        'if you want to move a key to a different playlist then use `move-key`'
    );
...
async function moveKeysCommand(
  ...
  const removedPlaylistsSet = new Set<string>();
  let keyObj;
  let keyName;
  const playlistArray = xdb.playlistArray;
  const playlistArrayUpper = playlistArray.map((val: string) => val.toUpperCase());
...
export { runMoveItemCommand, moveKeysWrapper };
