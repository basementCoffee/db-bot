...
import { getPlaylistArray } from './playlist';
...
async function getTitle(queueItem: any, cutoff?: number): Promise<string> {
  ...
  try {
    if (queueItem.type === StreamType.SPOTIFY) {
      if (isEmptyQueueItem) {
        try {
          ...
          queueItem.infos = (await getPlaylistArray(queueItem.url, StreamType.SPOTIFY))[0];
        }
      }
    ...
    processStats.debug('broken_url: ', queueItem.url, '\n', e, 'queueItem infos: ', queueItem.infos);
    title = 'broken_url';
  }
 ...
 */
function isShortCommand(guild: Guild, statement: string): boolean {
...
function isShortCommandNoArgs(args: Array<string>, guild: Guild, statement: string): boolean {
  return !args[0] && isShortCommand(guild, statement);
}
